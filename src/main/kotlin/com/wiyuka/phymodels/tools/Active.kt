...
class Active: Listener {
    ...
    fun onBlockBreak(e: BlockBreakEvent) {
        ...
        val centerXFixed = centerX + 0.5
        val centerY = (yMax + yMin) / 2.0
        val centerYFixed = centerY + 0.5
        val centerZ = (zMax + zMin) / 2.0
        val centerZFixed = centerZ + 0.5

        val centerLoc = Location(blockLoc.world, centerXFixed, centerYFixed, centerZFixed)
...
//        val modelCenter = Vector3i(e.player.x.toInt(), e.player.y.toInt(), e.player.z.toInt())
        val modelCenter = Vector3i(centerX.toInt(), centerY.toInt(), centerZ.toInt())
...
//        val modelGeometricCenter = model.getGeometricCenter(centerLoc, 1f)
        ...
        val modelEntity = ModelManager.generateModel(
            ...
            centerLoc.toLocation(e.player.world),
            1f,
            BodyType.PHYSICAL,
            e.player.world.name
        )
