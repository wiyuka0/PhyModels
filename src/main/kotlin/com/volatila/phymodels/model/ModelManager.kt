...
import com.volatila.phymodels.physics.obj.BlockUnit
import com.volatila.phymodels.physics.materialattributes.MaterialMass
...
class ModelManager {
    companion object{
        ...
        fun generateModel(model: Model, location: Location, scale: Float, modelType: BodyType, modelScriptData: ModelScriptData? = null): ModelEntity {
            val modelDisplayBlocks: HashMap<Vector3i, BlockUnit> = HashMap()
            spawnModelDisplayBlocks(location, model, scale).map {
                modelDisplayBlocks.put(it.key, BlockUnit(it.value))
            }
            ...
            when (modelType){
                ...
                BodyType.SCRIPTED -> {
                    modelEntity = getScriptedModel(modelScriptData, model, location, modelDisplayBlocks, scale)
                }
                BodyType.PHYSICAL -> {
                    modelEntity = generatePhysicalModel(model, location, modelDisplayBlocks, scale)
                }
            }
        ...
        private fun generatePhysicalModel(
            ...
            modelInfo.first.userObject = "model-${modelEntity.uniqueId}"

            modelInfo

            ObjectManager.addObjectToPhysicsWorld(modelInfo.first)
        ...
        private fun getScriptedModel(
            ...
            ObjectManager.addObjectToPhysicsWorld(modelRigidBody)
        ...
        fun spawnModelDisplayBlocks(spawnOrigin: Location, model: Model, scale: Float): HashMap<Vector3i, BlockDisplay> {
            ...
            for ((locRel, material) in model.blocks) {
         ...
         * 重新计算中心点至最接近几何中心的整数点
         */
        private fun fixCenter(blockListVector3i: MutableMap<Vector3i, Material>, modelWorldPosition: Vector3f): Pair<MutableMap<Vector3i, Material>, Vector3f> {
            val newBlockListVector3i = mutableMapOf<Vector3i, Material>()
            ...
            return newBlockListVector3i to Vector3f(modelCenter.x + modelWorldPosition.x, modelCenter.y + modelWorldPosition.y, modelCenter.z + modelWorldPosition.z)
        }
        ...
        TODO:
          1. 创建一个最大外接立方体
          2. 评估缩小每个轴所减少的非法位置作为这个轴的价值(v)
          3. 寻找+X +Y +Z -X -Y -Z中价值最高的轴
          4. 缩小价值最高
...
// 最终计算偏移量
            ...
            val modelRigidBody = PhysicsRigidBody(compoundShape, mass).apply {
                this.scheduler {
                    setPhysicsLocation(fixedSpawnLoc.toVector3f().toJmeVector3f())
        ...
        private fun buildModelShape(
            ...
            if(rectangleSize != null) {
                val boxCollisionShape = BoxCollisionShape(com.jme3.math.Vector3f(
...
//            PhyUtil.removeRigidBody(rigidBody)
            ObjectManager.removeObjectFromPhysicsWorld(rigidBody)
